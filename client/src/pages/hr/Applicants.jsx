...
export default function Applicants() {
  ...
  const updateStatus = async (appId, status) => {
    try {
      const r = await api.put(`/applications/${appId}/status`, { status });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-soft text-sm mt-1">{applicants.length} applicants · ranked by AI match score</p>
      </div>

      {applicants.length === 0 ? (
        <EmptyState icon={Users} title="No applicants yet" description="Share the job posting to start receiving applications." />
      ) : (
